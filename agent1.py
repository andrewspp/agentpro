...
En tant qu'économètre et chercheur académique, vous êtes chargé de conceptualiser une étude économétrique rigoureuse. Votre analyse doit respecter les standards des publications académiques en économie. 

IMPORTANT : Votre réponse doit être ENTIÈREMENT EN FRANÇAIS, y compris tous les termes techniques, équations, et références.
...
2. **REVUE DE LITTÉRATURE SOMMAIRE**
   - Mentionnez 5-7 références théoriques fondamentales (auteurs, théories) pertinentes pour cette analyse
...
6. **VARIABLES CLÉS ET TRANSFORMATIONS**
   ...
   - Discutez des problèmes potentiels de multicolinéarité

Votre réponse doit être structurée, précise et académique, intégrant la terminologie économétrique appropriée en français. Les équations doivent être formellement spécifiées. Limitez-vous aux méthodes compatibles avec les données disponibles.
"""
...
def parse_llm_output(output):
    ...
    intro_pattern = r"(?:##\s*|)INTRODUCTION ACADÉMIQUE:?([\s\S]*?)(?=##\s*|REVUE DE LITTÉRATURE|HYPOTHÈSES DE RECHERCHE|MÉTHODOLOGIE PROPOSÉE|LIMITES MÉTHODOLOGIQUES|VARIABLES CLÉS|$)"
    lit_review_pattern = r"(?:##\s*|)REVUE DE LITTÉRATURE:?([\s\S]*?)(?=##\s*|INTRODUCTION ACADÉMIQUE|HYPOTHÈSES DE RECHERCHE|MÉTHODOLOGIE PROPOSÉE|LIMITES MÉTHODOLOGIQUES|VARIABLES CLÉS|$)"
    hypotheses_pattern = r"(?:##\s*|)HYPOTHÈSES DE RECHERCHE:?([\s\S]*?)(?=##\s*|INTRODUCTION ACADÉMIQUE|REVUE DE LITTÉRATURE|MÉTHODOLOGIE PROPOSÉE|LIMITES MÉTHODOLOGIQUES|VARIABLES CLÉS|$)"
    methodology_pattern = r"(?:##\s*|)MÉTHODOLOGIE PROPOSÉE:?([\s\S]*?)(?=##\s*|INTRODUCTION ACADÉMIQUE|REVUE DE LITTÉRATURE|HYPOTHÈSES DE RECHERCHE|LIMITES MÉTHODOLOGIQUES|VARIABLES CLÉS|$)"
    limitations_pattern = r"(?:##\s*|)LIMITES MÉTHODOLOGIQUES:?([\s\S]*?)(?=##\s*|INTRODUCTION ACADÉMIQUE|REVUE DE LITTÉRATURE|HYPOTHÈSES DE RECHERCHE|MÉTHODOLOGIE PROPOSÉE|VARIABLES CLÉS|$)"
    variables_pattern = r"(?:##\s*|)VARIABLES CLÉS:?([\s\S]*?)(?=##\s*|INTRODUCTION ACADÉMIQUE|REVUE DE LITTÉRATURE|HYPOTHÈSES DE RECHERCHE|MÉTHODOLOGIE PROPOSÉE|LIMITES MÉTHODOLOGIQUES|$)"
...
def main():
    ...
    parser.add_argument("--backend", default="ollama", help="Backend LLM: 'ollama' (local) ou 'gemini'")
